...
public class game
{
	...
	private void checkForInjuries(int quarterNum, int timeRemaining)
	{
		...
		{
			int injuryRate = (playingAwayTeam.get(i).getDurabilityRating()*awayTeam.getTrainer().getInjuryPrevention());
		...
		{
			int injuryRate =  (playingHomeTeam.get(i).getDurabilityRating()*homeTeam.getTrainer().getInjuryPrevention());
			int injury = r.nextInt(injuryRate);
			if(injury == 500)
			{
	...
	private currentTeam substitutions(currentTeam playing, team fullTeam, boolean b, int quarterNum)
	{
		...
		for(int i  = 0; i<playing.length;i++)
		{
				...
				for(int j = 0; j<fullTeam.getSize();j++)
				{
					if(i+1 == fullTeam.getPlayer(j).getPosition())
					{
						if(fullTeam.getPlayer(j).isStarter() && !fullTeam.getPlayer(j).isInjured() && acceptableAmountOfFouls(fullTeam.getPlayer(j),quarterNum))
						{
							...
							if(fullTeam.getPlayer(j).getStamina() > playing.coach.getStaminaSubOut() && acceptableAmountOfFouls(fullTeam.getPlayer(j),quarterNum))
							{
								bob = true;
							}
						}
			...
			if(playing.get(i).isInjured() || playing.get(i).getStamina() < playing.coach.getStaminaSubIn() || bob || !acceptableAmountOfFouls(playing.get(i),quarterNum))
			{
