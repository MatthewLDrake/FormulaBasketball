...
public class game
{
	...
	{
	    int injuryRate = (playingAwayTeam[i].getDurabilityRating()*13000);
	    int injury = r.nextInt(injuryRate);
	    if(injury == 500)
	    {
		playingAwayTeam[i].setInjured(true);
		new Injury(playingAwayTeam[i],awayTeam, homeTeam, quarterNum, timeRemaining);
	    }
	}
	for(int i = 0; i < playingHomeTeam.length; i++)
	{
	    int injuryRate =  (playingHomeTeam[i].getDurabilityRating()*13000);
	    int injury = r.nextInt(injuryRate);
	    if(injury == 500)
	    {
		playingHomeTeam[i].setInjured(true);
		new Injury(playingHomeTeam[i],awayTeam, homeTeam, quarterNum, timeRemaining);
	    }
	}
	...
	for(int i  = 0; i<playing.length;i++)
	{
		...
		while(true)
		{
		    ...
		    else if(!playing[i].isInjured() && !useOtherPositions)
		    {		   
